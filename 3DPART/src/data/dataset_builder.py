...
class DatasetBuilder:
    ...
    def __init__(self, partnet_root: str, output_dir: str,
             ...
             num_parts_to_remove: int = 1,
             part_selection_mode: str = "random_distinct"):
        ...
        self.num_parts_to_remove = max(1, int(num_parts_to_remove))
        self.part_selection_mode = part_selection_mode
    ...
    def _process_sample(self, sample_dir: str, sample_id: str) -> Optional[dict]:
        ...
        selected_groups = self._select_remove_groups(part_groups)
        ...
        meta = create_sample_meta(
            ...
            removed_part_count=len(selected_groups),
            removed_obj_groups=selected_groups,
        )
    ...
    def _select_remove_groups(self, part_groups):
        """
        Select one or more distinct semantic part groups to remove.
        For the pilot experiment we just sample distinct groups uniformly.
        """
        if not part_groups:
            return []
    
        k = min(self.num_parts_to_remove, len(part_groups))
    
        if self.part_selection_mode == "random_distinct":
            selected = random.sample(part_groups, k)
        else:
            selected = random.sample(part_groups, k)

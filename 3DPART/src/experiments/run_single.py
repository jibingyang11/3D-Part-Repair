...
from ..repair.mlp_patch import mlp_patch_repair
...
def run_single_experiment(
    ...
    Methods:
      ...
      7. mlp_patch_rpa              (ours, learning-based patch + RPA target)
    """
    ...
    # --- 7. MLP Patch + RPA (ours, learning-based) ---
    try:
        res = mlp_patch_repair(damaged_mesh, target_loops_rpa,
                               generator=mlp_generator)
        results["mlp_patch_rpa"] = _eval(res, target_loops_rpa)
        if save_meshes and output_dir:
            _save(res, output_dir, sample_id, "mlp_patch_rpa")
        del res
    except Exception as e:
        results["mlp_patch_rpa"] = {"error": str(e)}

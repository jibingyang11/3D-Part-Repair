...
# Open3D rendering
# ═══════════════════════════════════════════════════════════════
def _save_o3d_transparent(vis, save_path, bg_rgb=(255, 255, 255), tol=3):
    """
    Capture Open3D screen and convert pure-white background to transparent PNG.
    """
    vis.poll_events()
    vis.update_renderer()
    ...
    r0, g0, b0 = bg_rgb
    mask = (
        (np.abs(img[..., 0].astype(np.int16) - r0) <= tol) &
        (np.abs(img[..., 1].astype(np.int16) - g0) <= tol) &
        (np.abs(img[..., 2].astype(np.int16) - b0) <= tol)
    )
    rgba[mask, 3] = 0
...
def render_o3d_interactive(geometries, save_path, camera_path,
    ...
    def save_callback(vis_):
        ...
        _save_o3d_transparent(vis_, save_path)
...
def render_o3d(geometries, save_path, camera_path=None,
    ...
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    _save_o3d_transparent(vis, save_path)
    vis.destroy_window()
...
def render_mpl_comparison(meshes, titles, save_path,
    ...
    plt.close(fig)
def _select_removed_components(removed_mesh, component_indices):
    parts = _safe_split_mesh(removed_mesh)
    parts = sorted(
        parts,
        key=lambda m: (
    ...
    for idx in component_indices:
        if idx < 0 or idx >= len(parts):
            raise IndexError(f"component index {idx} out of range")
        selected.append(parts[idx])
    return selected


def _merge_mesh_components(mesh_list):
    if len(mesh_list) == 1:
        return mesh_list[0]
    vertices = []
    faces = []

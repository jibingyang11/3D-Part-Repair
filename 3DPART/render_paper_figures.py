...
def _select_removed_leg_component(removed_mesh, leg_index=0):
    """
    从 removed_part_mesh 的连通分量中选出一条腿。
    为了稳定，按中心点排序后再取 leg_index。
    """
    ...
    if leg_index < 0 or leg_index >= len(parts):
        raise IndexError(
            f"leg_index={leg_index} 超出范围。removed_part_mesh 只有 {len(parts)} 个连通分量。"
        )

    return parts[leg_index]


def _build_single_leg_damaged_mesh(complete_mesh, removed_leg_mesh, round_digits=6):
    """
    从 complete_mesh 中真正删掉 removed_leg_mesh 对应的三角面，
    得到只缺一条腿的 damaged_mesh。
    """
    complete_v = np.asarray(complete_mesh.vertices)
    complete_f = np.asarray(complete_mesh.faces)
    removed_v = np.asarray(removed_leg_mesh.vertices)
    ...
    # 找到 removed_leg_mesh 对应到 complete_mesh 上的顶点索引
...
def render_sample(sample_id, cfg, out_dir,
    ...
    # ==========================================================
    # 关键修改：
    # 先从 removed_part_mesh 里选一条腿，
    # 再从 complete_mesh 里真正删掉这条腿，生成新的 damaged_mesh
    # ==========================================================
    removed = _select_removed_leg_component(removed_all, leg_index=leg_index)
    damaged = _build_single_leg_damaged_mesh(complete, removed)
    ...
    if HAS_OPEN3D:
        ...
        camera_path = os.path.join(out_dir, f"{sid}_camera.json")
        if reset_camera and os.path.exists(camera_path):
            os.remove(camera_path)

        # 第一张图：交互式设置视角
        first_save = os.path.join(out_dir, f"{sid}_complete.png")
        ...
        render_o3d(
            ...
            os.path.join(out_dir, f"{sid}_damaged.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d(
            ...
            os.path.join(out_dir, f"{sid}_removed_part.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_patch(
            ...
            C_MESH, C_PATCH_CF,
            os.path.join(out_dir, f"{sid}_repaired_cf.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_patch(
            ...
            C_MESH, C_PATCH_PL,
            os.path.join(out_dir, f"{sid}_repaired_planar.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_patch(
            ...
            C_MESH, C_PATCH_AF,
            os.path.join(out_dir, f"{sid}_repaired_adv_front.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_patch(
            ...
            C_MESH, C_PATCH_LH,
            os.path.join(out_dir, f"{sid}_repaired_lh.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_loops(
            ...
            os.path.join(out_dir, f"{sid}_boundary_loops.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_bbox(
            ...
            os.path.join(out_dir, f"{sid}_target_loops.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_loops(
            ...
            os.path.join(out_dir, f"{sid}_failure_rpa_target.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_loops(
            ...
            os.path.join(out_dir, f"{sid}_failure_lh_target.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_patch(
            res_planar["repaired_mesh"], res_planar["new_faces"],
            C_MESH, C_PATCH_PL,
            os.path.join(out_dir, f"{sid}_failure_rpa_patch.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_patch(
            res_lh["repaired_mesh"], res_lh["new_faces"],
            C_MESH, C_PATCH_LH,
            os.path.join(out_dir, f"{sid}_failure_lh_patch.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_o3d_with_patch(
            res_planar["repaired_mesh"], res_planar["new_faces"],
            C_MESH, C_PATCH_PL,
            os.path.join(out_dir, f"{sid}_patch_overlay.png"),
            camera_path=camera_path,
            width=width,
            height=height
        )
        ...
        render_mpl(
            complete,
            os.path.join(out_dir, f"{sid}_complete.png"),
    ...
    render_mpl_comparison(
        ...
        save_path=os.path.join(out_dir, f"{sid}_comparison_4col.png"),
        patch_faces_list=[
            None,
            res_cf["new_faces"],
...
def main():
    ...
    # 单腿选择
    parser.add_argument("--leg_index", type=int, default=0,
                        help="Which boundary loop / leg to use (default: 0)")
    ...
    print(f"Single-leg target index: {args.leg_index}")
